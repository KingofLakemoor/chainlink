...
let content = fs.readFileSync('src/components/pickem/CharityBanner.tsx', 'utf-8');

const targetContent = `  const [progress, setProgress] = useState<{ raised: number; goal: number; pot: number; maxPot: number } | null>(null);
...
const importTarget = "import { ExternalLink, Heart, Trophy, Calendar, Gift, CheckCircle, TrendingUp } from 'lucide-react';";
const importReplacement = "import { ExternalLink, Heart, Trophy, Calendar, Gift, CheckCircle } from 'lucide-react';\nimport { CharityProgressTracker } from './CharityProgressTracker';";

content = content.replace(importTarget, importReplacement);

const htmlTarget = `            {/* PROGRESS BARS */}
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex justify-between text-sm font-medium mb-2">
                    <span className="text-zinc-300">Club 602 Prize Pot (Max {progress ? formatCurrency(progress.maxPot) : '$500'})</span>
...
const htmlReplacement = `            {/* PROGRESS BARS */}
...
fs.writeFileSync('src/components/pickem/CharityBanner.tsx', content);
